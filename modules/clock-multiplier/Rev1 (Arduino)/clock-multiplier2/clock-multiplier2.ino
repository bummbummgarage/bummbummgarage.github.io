...
// Distribution
const int distributionCVPin = A2;
...
int getQuantity() {
    int q;
    q = map( analogRead(quantityCVPin), 0, 1023, 1, ( maxQuantity + 1 ) );
    return q;
}
...
int getDistribution() {
    int d;
    d = map( analogRead(distributionCVPin), 0, 1023, 1, ( maxDistribution + 1 ) );
    return d;
}
